...
\begin{figure}[h]  % 图片
  ...
  \caption{Flow Chart of Our Work} \label{fig:2}  % 标题与标签
  \end{figure}  % 图片结束
...
To ensure that the measurements are non-destructive, inexpensive, and can be done by a small number of people using minimal tools while maximizing the accuracy of the measurements, the following tools and measurement methods are chosen (Some of the tools can be referenced in Fig.2):
...
\begin{figure}[h]  % 图片
  \small
  \centering  % 居中
  \includegraphics[width=12cm]{3-Measuring tools.png}
...
\begin{figure}[h]  % 图片
  \small
  \centering  % 居中
  \includegraphics[width=10cm]{4-Infrared foot pressure zoning map.png}
...
Where $\varepsilon$  represents the adjustment coefficient, and $\mu$ represents the coefficient of kinetic friction.The $F$ obtained above is used as an independent variable in the wear volume model to further calculate the material wear amount $W$:
...
\begin{figure}[h]  % 图片
  \small
  \centering  % 居中
  \includegraphics[width=10cm]{5-Establishment of stair coordinate axes and empirical partitioning of tread surfaces.png}
...
From the above equation, We established the functional relationship between the deflection Wr and the horizontal coordinate of the force application point X and plotted the corresponding Figure 5. The Figure shows that the deflection reaches its maximum value near X=Xmax.

\begin{figure}[h]  % 图片
  \small
  \centering  % 居中
  \includegraphics[width=10cm]{8-The depth of the depression on the staircase caused by the bending deformation.jpg}
...
Where $L_1$ refers to The width corresponding to the indentation caused by each normal load.($b$ and $L_1$ are specifically indicated in Figure 7)


\begin{figure}[h]  % 图片
  \small
  \centering  % 居中
  \includegraphics[width=9cm]{9-Length of catenary line-x.png}
  \caption{Length of catenary line} \label{fig:2}  % 标题与标签
  \end{figure}  % 图片结束


  Finally,the calculated wear width resulting from every 2,000 actions is \( 1.4 \pm 0.015 \, \text{nm} \).


\begin{figure}[h]  % 图片
  \small
  \centering  % 居中
  \includegraphics[width=9cm]{7-Bending model diagram.png}
  ...
  \begin{figure}[h]  % 图片
    \small
    \centering  % 居中
    \includegraphics[width=10cm]{6-Foot sole grid.png}
...
\]
  ...
  On this basis, the standard coordinates of the $i$+1-th stair need to be adjusted according to the constraints of the comfort zone. The specific calculation process is shown in Figure 9. 
  
  
  \begin{figure}[h]  % 图片
    \small
    \centering  % 居中
    \includegraphics[width=12cm]{10-The flowchart for calculating the cumulative number of landing points.png}
  ...
  \begin{figure}[h]  % 图片
    \small
    \centering  % 居中
    \includegraphics[width=12cm]{11-The Distribution of the Number of Landing Points of M,H and C.png}
...
\subsection{Wear depth analysis of a single step}
  ...
  Where \( G = 1624 \pm 19.2 \, \text{N} \) (from \cite{YISX202303023})%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

  
  Where $FG_i$ is the vertical pressure exerted by region $i$ on the stairs and $G$ is the total vertical pressure.
  ...
  The wear analysis for forefoot walking closely resembles that of full-foot walking; however, the proportional force distribution differs significantly. This is because forefoot walking involves only two main force-bearing regions applying pressure. As such, a detailed explanation is omitted here due to space constraints. The depth of wear was calculated using the same method as for full-foot walking, with adjustments made solely to the parameters.
  \end{itemize}
...
\State \textbf{for} $K_c \in \text{Range}(100, 100000)$ \textbf{do}
    ...
    \State \hspace{1em} $\text{Result}[i] = \left[ \mathcal{E}-\mathcal{U}, K_c, K_D \right]$
    \State \hspace{1em} $i = i + 1$
\State \textbf{end}
\State Find the minimum $\text{Result}[n][1]$
...
Using this expression, we can calculate the total number of people on each path.
...
Thus, the average time interval between uses of the stairs is obtained.
...
The wear in the 39-48 range is primarily influenced by environmental factors, exhibiting a Micro-weathering effect, and it is assumed that no significant wear is caused to the stairs.
...
Next, we conduct secondary sampling on the treads with abnormal depression points, increasing the sampling density to 300 samples per square meter. We repeat the abnormal point analysis on the new data. If the abnormal points show circular or strip-shaped clustering (as seen in Figure 12), we preliminarily conclude that the area may have been repaired with polymer reinforcement methods.
\begin{figure}[h]  % 图片
  \small
  \centering  % 居中
  \includegraphics[width=11cm]{14-Illustrative cluster diagram of the renovated areas.png}
...
Maintaining the total number of people unchanged, by changing the ratio of $K_{\text{upstairs}}$ to $K_{\text{downstairs}}$ and simulating, the shapes of the depressions under different parameters can be obtained（Fig.13）.
...
\begin{figure}[h]  % 图片
  \small
  \centering  % 居中
  \includegraphics[width=11cm]{15-Staircase section Worn Pit Profile Line varies with the ratio of the number of passengers on the upward journey..png}
